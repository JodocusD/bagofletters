...
library(DBI)
publisher <- 2
startdate <- c('2016-01-01')
myquery <- paste("SELECT  `date`, `ad_slot_id`, `channel_id`, `count` FROM `statistics_slots` WHERE `ad_slot_id` IN( '2',  '13',  '22',  '33', '381', '377', '380', '378', '379',  '42',  '52',  '66',  '63',  '61', '317', '144', '234', '240', '242', '246', '244', '248', '250', '216', '313', '318', '230', '210') AND `date` > '", startdate, "' AND `publisher_id` = ", publisher , sep ='')
...
ggplot(data = singlesect, aes(date, count, col = section)) +
  geom_point(alpha = .8) +
  labs(title = paste('Daily PageViews for', sectionofinterest, sep = ' '), y = "pageviews")  +
  theme_light() +
  theme(axis.text.x = element_text(angle = 90), legend.position="none") +
  scale_y_continuous(breaks = seq(0, maxyax_singlsect, by = 200000), limits = c(0, maxyax_singlsect)) +
  scale_color_manual(values = cols) +
  facet_grid(section ~ medium)
